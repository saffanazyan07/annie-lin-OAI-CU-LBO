...
#include "openair3/NGAP/ue_tunnel_info.h"
#pragma pack(1)
...
// uint8_t pdcp_type[1] = {0x80};
uint8_t annie[3] = {0x80, 0x00, 0x00};
uint8_t annierep[3] = {0x80, 0x00, 0x00};
int lbo_instance;
int lboudpid1;
int broadcastid;
int i = 0;
int j = 0;
int const arr_size = 5010;
int rawsocket[2];
...
static  int createrawsocket(int rawsocket[]) {
  int raw_socketfd = socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
  if (raw_socketfd < 0){
    perror("socket");
    exit(EXIT_FAILURE);
  }
  ...
  strncpy(ifr.ifr_name, "rename2", IFNAMSIZ);
  if (ioctl(raw_socketfd, SIOCGIFINDEX, &ifr) == -1) {
    perror("ioctl");
    exit(1);
  }
  ...
  if (bind(raw_socketfd, (struct sockaddr *)&sll, sizeof(sll)) == -1){
    perror("bind");
    exit(1);
  }

  rawsocket[0] = raw_socketfd;
  rawsocket[1] = ifr.ifr_ifindex;
}

static int lboudpsocket(const char* ipAddress, int PORT){
  int udp_sockfd;
  ...
  if ((udp_sockfd = socket(AF_INET, SOCK_DGRAM, 0)) == -1) {
    ...
    if (bind(udp_sockfd, (const struct sockaddr*)&server_address, sizeof(server_address)) == -1) {
        perror("Bind failed");
        exit(EXIT_FAILURE);
    }
...
static int broadcast_socket(const char* BROADCAST_IP, int PORT){
    int sockfd;
    ...
    recv_addr.sin_addr.s_addr = inet_addr(BROADCAST_IP);

    // Bind socket to any available network interface
...
void addHex(unsigned char* hexNum, int size) {
    int carry = 1;  

    for (int i = size - 1; i >= 0; i--) {
        int sum = hexNum[i] + carry;
        hexNum[i] = sum % 256;  
        carry = sum / 256;  
    }
}

void *receive_LBO_packet (void *arg){
  while(1){  
    uint8_t datarecv[65536];
      ssize_t packet_size = recv(rawsocket[0], datarecv, sizeof(datarecv), 0);
      if (packet_size == -1) {
        perror("recv");
        exit(1);
      }

       uint8_t smac[6] = {0x18, 0x31, 0xbf, 0x93, 0x76, 0xca};
      // uint8_t dmac[6] = {0x88, 0xd7, 0xf6, 0xd4, 0xc7, 0xfa};
      if (memcmp(&datarecv[6], smac, 6) == 0) {
          uint8_t datarecv2[1024];
          memcpy(&datarecv2[0], annie, 3);
          memcpy(&datarecv2[3], &datarecv[14], packet_size-14);
          annie[2]+=1;

      //printf("LBO send to DU\n");
            
          extern in_addr_t annie_dl_ipv4;
          extern uint32_t annie_dl_teid;
          // char search[10];
          // strncpy(search, des_ip + 8, 2);
          // search[10] = '\0';
          // //printf("Src IP address: %s\n", search);
          // int n;
          // int search_num = atoi(search);
          // n = search_num - 1;
          gtpv1uCreateAndSendMsg(
                lbo_instance, annie_dl_ipv4, 2152, GTP_GPDU, annie_dl_teid, datarecv2, packet_size-11, 0, 0, 0, 0, NO_MORE_EXT_HDRS, NULL, 0);
        }
      }
    } 
...
void *receive_packet (void *arg){
  while(1){  
    uint8_t datarecv[2048];
    struct sockaddr_in client_address;
    socklen_t len = sizeof(client_address);

    // Receiving data using recvfrom
    //printf("---------received---------\n");
    ssize_t packet_size = recvfrom(lboudpid1, datarecv, sizeof(datarecv), 0, (struct sockaddr*)&client_address, &len);
    //printf("packet_size: %zd\n",packet_size);
    if (packet_size == -1) {
      perror("recvfrom");
      exit(1);
    }
    struct iphdr *iph = (struct iphdr*)datarecv;

    // Convert the binary destination IP address to human-readable form
    struct in_addr des_addr;
    des_addr.s_addr = iph->daddr;
    char des_ip[INET_ADDRSTRLEN];
    inet_ntop(AF_INET, &des_addr, des_ip, INET_ADDRSTRLEN);
    ...
    if (packet_size > 0){
    //dump_data(datarecv, packet_size);

     uint8_t datarecv2[2048];
     // memcpy(&datarecv2[0], pdcp_type, 1);
     memcpy(&datarecv2[0], annie, 3);
     memcpy(&datarecv2[3], &datarecv[0], packet_size+3);
     // int size = sizeof(annie) / sizeof (annie[0]);
     // addHex(annie, size);
     annie[2]+=1;
     if(annie[2] == 0){
       annie[1]+=1;
     ...
     if(get_dl_info_by_ue_ip(des_ip, &DL_IP, &DL_TEID)) {
     gtpv1uCreateAndSendMsg(
           lbo_instance, DL_IP, 2152, GTP_GPDU, DL_TEID, datarecv2, packet_size+3, 0, 0, 0, 0, NO_MORE_EXT_HDRS, NULL, 0);}
     else{
      printf("UE_IP: %s not found\n", des_ip);
     }
    }
  }
}

struct forwarding_table storedTable[1024];
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;

void *receive_broadcastpacket (void *arg){
  struct forwarding_table buffer[1024];
  int i = 0;

  while (1) {
    // Receive broadcast message
    ssize_t recv_len = recvfrom(broadcastid, buffer, sizeof(buffer), 0, NULL, 0);
    if (recv_len == -1) {
        perror("recvfrom");
        exit(EXIT_FAILURE);
    }
    ...
    int index = -1;
    for (int k= 0; k < i; ++k) {
      int update = strcmp(storedTable[k].ue_ip, buffer[0].ue_ip);
      if (update == 0){
        index = k;
        break;
      }
    ...
    if (index!= -1){
      strcpy(storedTable[index].ue_ip, buffer[0].ue_ip);
    strcpy(storedTable[index].cu_ip, buffer[0].cu_ip);
    }
    else{
      strcpy(storedTable[i].ue_ip, buffer[0].ue_ip);
    strcpy(storedTable[i].cu_ip, buffer[0].cu_ip);
      i++;
    }

    // printf("--------------------------------------------\n");
    // for (int j= 0; j < i; ++j) {
    //     printf("ue ip: %s, cu ip: %s\n", storedTable[j].ue_ip, storedTable[j].cu_ip);
    // }
...
double T1array[arr_size];
double T2array[arr_size];
...
static void gtpv1uSend(instance_t instance, gtpv1u_tunnel_data_req_t *req, bool seqNumFlag, bool npduNumFlag) {
  ...
  //     int ready = select(lboudpid1 + 1, &read_fds, NULL, NULL, &timeout);
  ...
  //     } else if (ready == 0) {
  //         // Timeout occurred
  //         printf("Timeout: No data received within 1m seconds\n");
  //         break;
  //     } else{   
  //     uint8_t datarecv[2048];
  //     struct sockaddr_in client_address;
  ...
  //      extern in_addr_t annie_dl_ipv4;
  //      extern uint32_t annie_dl_teid[2];
  //      gtpv1uCreateAndSendMsg(
  //            lbo_instance, annie_dl_ipv4, 2152, GTP_GPDU, annie_dl_teid[0], datarecv2, packet_size+3, seqNumFlag, npduNumFlag, 0, 0, NO_MORE_EXT_HDRS, NULL, 0);
  //     }
  //      break;
  //   }
  // }
  ...
  if (buffer[0] != 0x45){
    ...
    char localnet[] = "10.60.0.";
    int result = strncmp(des_ip, localnet, 8);
    char ueip[] = "60.60.0.101";
    int result3 = strncmp(des_ip, ueip, 11);

    if (result == 0){
      int k = 0;
      const char *cu_address = NULL;
      while (1) {
        pthread_mutex_lock(&mutex);

        if (k < 1024 && storedTable[k].ue_ip[0] != '\0') {
            int searchcu = strcmp(storedTable[k].ue_ip, des_ip);
            if (searchcu == 0) {
                cu_address = storedTable[k].cu_ip;
                pthread_mutex_unlock(&mutex);
                break;
            }
            ++k;
        } else {
            // Handle the case when the end of the storedTable is reached
            pthread_mutex_unlock(&mutex);
            break;
        }
      ...
      //dump_data(req->buffer+req->offset , length);
      struct sockaddr_in dest_address;

      memset(&dest_address, 0, sizeof(dest_address));

      dest_address.sin_family = AF_INET;
      dest_address.sin_addr.s_addr = inet_addr(cu_address);
      dest_address.sin_port = htons(6666);

      //printf("cu_address: %s\n", cu_address);
      sendto(lboudpid1, req->buffer+req->offset , length, 0, (const struct sockaddr*)&dest_address, sizeof(dest_address));
  }      
   

  else if(result3 == 0){ 
    uint8_t data[1024];
    uint8_t smac[6] = {0x88, 0xd7, 0xf6, 0xd4, 0xc7, 0xfa};
    uint8_t dmac[6] = {0x18, 0x31, 0xbf, 0x93, 0x76, 0xca};
    memcpy (&data[0], dmac, 6);
    memcpy (&data[6], smac, 6);
    data[12] = 0x08;
    data[13] = 0x00;
    memcpy (&data[14], req->buffer+req->offset, req->length);
    int data_len = length + 14;
    //dump_data(data, data_len);

    struct sockaddr_ll dest_addr;
    memset(&dest_addr, 0, sizeof(dest_addr));
    dest_addr.sll_family = AF_PACKET;
    dest_addr.sll_ifindex = rawsocket[1];
    dest_addr.sll_halen = 6;
    dest_addr.sll_addr[0] = 0x18;
    dest_addr.sll_addr[1] = 0x31;
    dest_addr.sll_addr[2] = 0xbf;
    dest_addr.sll_addr[3] = 0x93;
    dest_addr.sll_addr[4] = 0x76;
    dest_addr.sll_addr[5] = 0xca;

    if (sendto(rawsocket[0], data, data_len, 0, (struct sockaddr*) &dest_addr, sizeof(dest_addr)) < 0) {
        perror("sendto");
        exit(EXIT_FAILURE);
    }
      ...
      //     // n = search_num - 1;
      //     gtpv1uCreateAndSendMsg(
      //           lbo_instance, annie_dl_ipv4, 2152, GTP_GPDU, annie_dl_teid[0], datarecv2, packet_size-11, seqNumFlag, npduNumFlag, 0, 0, NO_MORE_EXT_HDRS, NULL, 0);
      //   }
  }
  ...
  //     //clock_gettime(CLOCK_MONOTONIC, &start1);
  //     extern in_addr_t annie_dl_ipv4;
  //     //printf("ip:%s\n", inet_ntoa(*((struct in_addr*)&annie_dl_ipv4)));
  //     extern uint32_t annie_dl_teid[2];
  //     char search2[10];
  //     strncpy(search2, des_ip + 8, 2);
  //     search2[10] = '\0';
  //     //printf("Des IP address: %s\n", search2);
  //     int n;
  //     int search2_num = atoi(search2);
  //     n = search2_num - 1;
  // //     if(buffer[20] == 0x08){
  //     if(search2_num == 1){
  //       uint8_t uedatareq[1024];
  //       memcpy(&uedatareq[0], annie, 3);
  //       memcpy(&uedatareq[3], req->buffer+req->offset, req->length);
  //       int uedatareq_len =  length + 3;
  //       annie[2]+=1;
  //       if(annie[2] == 0xff){
  //         annie[1]+=1;
  //       }
  //  //     dump_data(uedatareq, uedatareq_len);
  //       gtpv1uCreateAndSendMsg(
  //       lbo_instance, annie_dl_ipv4, 2152, GTP_GPDU, annie_dl_teid[n], uedatareq, uedatareq_len, seqNumFlag, npduNumFlag, 0, 0, NO_MORE_EXT_HDRS, NULL, 0);
  //   }
  // // else if(buffer[20] == 0x00){
  // else if(search2_num == 2){
  //       uint8_t uedatarep[1024];
  //       memcpy(&uedatarep[0], annierep, 3);
  //       memcpy(&uedatarep[3], req->buffer+req->offset, req->length);
  //       int uedatarep_len =  length + 3;
  //       annierep[2]+=1;
  //       if(annierep[2] == 0xff){
  //         annierep[1]+=1;
  //       }
  //    //   dump_data(uedatarep, uedatarep_len);
  //       gtpv1uCreateAndSendMsg(
  //       lbo_instance, annie_dl_ipv4, 2152, GTP_GPDU, annie_dl_teid[n], uedatarep, uedatarep_len, seqNumFlag, npduNumFlag, 0, 0, NO_MORE_EXT_HDRS, NULL, 0);
  //   }
  //  }
...
instance_t gtpv1Init(openAddr_t context) {
  pthread_mutex_lock(&globGtp.gtp_lock);
  char cu_ip[] = "192.168.50.110";
  int result1 = strncmp(context.originHost, cu_ip, 14);
  // char du_ip[] = "192.168.130.83";
  // int result2 = strncmp(context.originHost, du_ip, 14);
  ...
  if(result1 == 0){
    lbo_instance = id;
    lboudpid1 = lboudpsocket("192.168.50.93", 6666);
    broadcastid = broadcast_socket("192.168.50.255", 12345);
    createrawsocket(rawsocket); 
    
    pthread_t annie1;
    pthread_t annie2;
    pthread_t annie3;
    pthread_create(&annie1, NULL, receive_packet, NULL);
    pthread_create(&annie2, NULL, receive_broadcastpacket, (void*)storedTable);
    pthread_create(&annie3, NULL, receive_LBO_packet, NULL);
  }

  // if(result2 == 0){
  //   if(lboudpid2 == NULL){
  //     lboudpid2 = lboudpsocket("192.168.131.88", 6667);
  //   }
  // }
  pthread_mutex_unlock(&globGtp.gtp_lock);
